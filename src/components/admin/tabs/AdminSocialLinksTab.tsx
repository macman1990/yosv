...
export const AdminSocialLinksTab: React.FC = () => {
  ...
  const reorder = async (id: string, direction: 'up' | 'down') => {
    ...
    addToast('Link order updated', 'info');
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    addToast('Social link deleted', 'info');
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold text-white font-syne">Social Links</h2>
          <p className="text-xs text-zinc-400">Expose public channels without exposing the CMS itself.</p>
        </div>
        ...
        {socialLinks.map((link) => (
          <div key={link.id} className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-[#12141c] p-4">
            ...
            <div className="flex items-center gap-1.5">
              <button type="button" onClick={() => reorder(link.id, 'up')} className="rounded-lg bg-white/5 p-1.5 text-zinc-300 hover:bg-white/10"><ArrowUp className="h-3.5 w-3.5" /></button>
              <button type="button" onClick={() => reorder(link.id, 'down')} className="rounded-lg bg-white/5 p-1.5 text-zinc-300 hover:bg-white/10"><ArrowDown className="h-3.5 w-3.5" /></button>
              <button type="button" onClick={() => setEditing(link)} className="rounded-lg bg-white/5 p-1.5 text-zinc-300 hover:bg-white/10"><Edit2 className="h-3.5 w-3.5" /></button>
              <button type="button" onClick={() => handleDelete(link.id)} className="rounded-lg bg-rose-500/10 p-1.5 text-rose-400 hover:bg-rose-500/20"><Trash2 className="h-3.5 w-3.5" /></button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-[100000] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-xl rounded-3xl border border-white/15 bg-[#0f1015] p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between border-b border-white/10 pb-3">
              <h3 className="text-base font-bold text-white">Edit Social Link</h3>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs font-mono uppercase tracking-[0.18em] text-zinc-400">Platform</label>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-mono uppercase tracking-[0.18em] text-zinc-400">Label</label>
              ...
              <div className="space-y-1">
                <label className="text-xs font-mono uppercase tracking-[0.18em] text-zinc-400">Icon Key</label>
              ...
              <div className="flex items-center justify-between rounded-xl border border-white/10 bg-[#14161f] p-3">
                <span className="text-xs font-mono uppercase tracking-[0.18em] text-zinc-400">Visible on public site</span>
