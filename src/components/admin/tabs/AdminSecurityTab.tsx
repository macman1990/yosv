...
export const AdminSecurityTab: React.FC = () => {
  ...
  const { setIsAdminMode, addToast } = usePortfolio();
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setNewPassword('');
      setConfirmPassword('');
      addToast('Master key successfully updated.', 'success');
    } else {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200 max-w-2xl">
      <div className="border-b border-white/10 pb-4">
        <h2 className="text-xl font-bold text-white font-syne">Security & Master Key Settings</h2>
        <p className="text-xs text-zinc-400">
          Manage admin access credentials and session authorization
        </p>
      </div>
      ...
      <div className="p-7 rounded-3xl bg-[#12141c] border border-white/10 space-y-6">
        ...
        {success && (
          <div className="flex items-center gap-2 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs">
            ...
            <span>Master key has been successfully updated and encrypted.</span>
          </div>
        )}

        <form onSubmit={handleChangePassword} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-mono text-zinc-400">New Master Password</label>
            <input
              type="password"
              required
              value={newPassword}
          ...
          <div className="space-y-1">
            <label className="text-xs font-mono text-zinc-400">Confirm New Password</label>
            <input
              type="password"
              required
              value={confirmPassword}
              onChange={(e) => setConfirmPassword(e.target.value)}
              placeholder="Re-type new credential..."
              className="w-full px-4 py-2.5 rounded-xl bg-black/50 border border-white/10 focus:border-emerald-500 text-white text-xs font-mono"
            />
          </div>
      ...
      <div className="p-6 rounded-3xl bg-white/[0.02] border border-white/10 flex items-center justify-between">
        <div className="space-y-0.5">
          <h4 className="text-xs font-bold text-white uppercase font-mono">End Session</h4>
          <p className="text-xs text-zinc-400">Terminates active administrative authentication</p>
        </div>
        <button
          type="button"
          onClick={() => {
